...
class LIBMIKEY_API MikeyPolicyParam {
  public:
    MikeyPolicyParam(uint8_t type, uint8_t length, const uint8_t* value);
    ~MikeyPolicyParam();
    uint8_t  type;   // As defined above
    uint8_t  length; // Length of value in bytes
    uint8_t* value;  // type value
};
...
class LIBMIKEY_API MikeyPayloadSP : public MikeyPayload {
  public:
    ...
    MikeyPayloadSP(uint8_t* start_of_header, int lengthLimit);
    // Destructor
    ~MikeyPayloadSP() override;
    // Add one policy type with type, size & value
    void addMikeyPolicyParam(uint8_t type, uint8_t length, uint8_t* value);
