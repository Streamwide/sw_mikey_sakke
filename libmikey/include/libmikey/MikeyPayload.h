...
class LIBMIKEY_API MikeyPayload : public MObject {
  public:
    ...
    MikeyPayload(uint8_t* start_of_message);
    ~MikeyPayload() override;
  ...
  protected:
    ...
    uint8_t* startPtr; // Points to the first memory position
                      // within this payload.

    uint8_t* endPtr; // Points to the first memory position
                    // after this payload
