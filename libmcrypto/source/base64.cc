...
LIBMCRYPTO_API string base64_encode(const unsigned char* input, int length) {
    int                               i;
    int                               length_complete_blocks = length - length % 3;
    string                            output                 = "";
    static constexpr string_view alphabet               = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    ...
    switch (length % 3) {
        ...
        case 2:
            ...
            output += alphabet[((input[length - 2] << 4) | (input[length - 1] >> 4)) & 0x3F];
            output += alphabet[(input[length - 1] << 2) & 0x3F];
            output += '=';
            return output;
    }
...
LIBMCRYPTO_API unsigned char* base64_decode(const string& input, int* output_length) {
...
#undef XX
    ...
    if (input_cpy.length() % 4) {
        MIKEY_SAKKE_LOGE("Invalid base64 input");
        return nullptr;
    }
    ...
    if (output == nullptr) {
        MIKEY_SAKKE_LOGE("Not enough memory to allocate output");
        return nullptr;
    }
...
LIBMCRYPTO_API unsigned char* base64_decode(unsigned char* input, int input_length, int* output_length) {
