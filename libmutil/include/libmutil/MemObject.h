...
class LIBMUTIL_API MObject {
  ...
  private:
    ...
    */
    ...
    /**
     * mutex copy constructor is deleted but we want to allow MemObject copy, so refLock has to be a pointer
     */
    std::mutex* refLock;
};
...
Pitfall 3: If objects reference each other in such a way that
                ...
                list will be freed.
*/
template <class OPType> class MRef {
...
template <class OPType> OPType MRef<OPType>::getPointer() const {
    return objp;
}

template <class OPType> void MRef<OPType>::setPointer(const OPType o) {
    if (o != NULL) {
        // check if the pointer is to an MObject ...
        if (dynamic_cast<OPType>(o) != NULL) {
            objp = o;
        } else {
            MIKEY_SAKKE_LOGE("MRef: Trying to use an MRef for a non MObject class");
            MIKEY_SAKKE_LOGE("Stack trace: %s", getStackTraceString().c_str());
        }
    } else {
        objp = NULL;
    }
...
template <class OPType> bool MRef<OPType>::init() {
    bool ret = false;
    if (objp != NULL) {
        ret = increase();
    }
...
template <class OPType> bool MRef<OPType>::increase() {
    bool ret = false;
    if (objp != NULL) {
        objp->incRefCount();
        ret = true;
...
template <class OPType> bool MRef<OPType>::decrease() {
    bool ret = false;
    ...
    if (objp != NULL) {
        ...
        if (rc <= 0) {
            if (rc < 0) {
#ifndef _MSC_VER
                MIKEY_SAKKE_LOGE("MRef::~MRef: WARNING: deleteing object with negative reference count (%d) - created without reference?",
                                 rc);
#endif
            }
            delete getPointer();
...
template <class OPType> MRef<OPType>::MRef(OPType optr) {
    setPointer(optr);
    init();
}

template <class OPType> MRef<OPType>::MRef(const MRef<OPType>& r) {
    setPointer(r.getPointer());
    increase();
}

template <class OPType> MRef<OPType>::~MRef() {
    decrease();
    setPointer(NULL);
}

template <class OPType> MRef<OPType>& MRef<OPType>::operator=(const OPType o) {
    decrease();
    setPointer(o);
    increase();
    return *this;
...
template <class OPType> MRef<OPType>& MRef<OPType>::operator=(const MRef<OPType>& r) {
    decrease();
    setPointer(r.getPointer());
    increase();
    return *this;
...
template <class OPType> bool MRef<OPType>::operator==(const MRef<OPType> r) const {
...
template <class OPType> bool MRef<OPType>::isNull() const {
    return getPointer() == NULL;
}

template <class OPType> MRef<OPType>::operator bool() const {
    return !isNull();
}

template <class OPType> OPType MRef<OPType>::operator->() const {
    OPType ret;
    ret = getPointer();
    if (ret == NULL) {
        assert(ret != NULL);
