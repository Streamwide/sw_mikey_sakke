...
LIBMUTIL_API std::vector<string> split(const string& s, bool do_trim, char delim, bool includeEmpty) {
    ...
    if (s[s.length() - 1] == delim && includeEmpty)
        ret.emplace_back("");
...
LIBMUTIL_API string itoa(int64_t i) {
    ...
    for (int j = 0; j < 16; j++)
        buf[j] = 0;
#ifdef _MSC_VER
    _snprintf_s(buf, 30, 29, "%" PRId64, i);
#else
    snprintf(buf, 30, "%" PRId64, i);
#endif
    return string(buf);
}

template <class charT, class traits, class Alloc>
LIBMUTIL_API int strCaseCmp(const std::basic_string<charT, traits, Alloc>& s1, const std::basic_string<charT, traits, Alloc>& s2,
